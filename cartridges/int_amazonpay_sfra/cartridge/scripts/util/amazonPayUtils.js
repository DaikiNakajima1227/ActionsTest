...
var AMAZON_SIGNATURE_ALGORITHM = 'AMZN-PAY-RSASSA-PSS-V2';
/**
...
function calculateSignature(stringToSign) {
    ...
    var amzPaySecretKey = preferences.amzPaySecretKey;
    var splitAmzPaySecretKey = amzPaySecretKey.split('-----BEGIN PRIVATE KEY-----\n')[1].split('-----END PRIVATE KEY-----')[0];
    var signedString = new Signature().signBytes(new Bytes(stringToSign), splitAmzPaySecretKey, 'SHA256withRSA/PSS');
    var encodedSignature = Encoding.toBase64(signedString);
...
AmazonPayUtils.getRecurringMetadata = function (basket) {
    var grossPrice = basket ? basket.getTotalGrossPrice() : null;
    var recurringMetadata = {};

    if (basket && basket.productQuantityTotal != 0) {
        var recurringProductLineItem = basket.getProductLineItems().toArray().find(function (pli) {
            return pli.product.custom.amzPayIsRecurringProduct;
        });

        recurringMetadata = {
            frequency: {
                unit: recurringProductLineItem.product.custom.amzPayRecurringUnit.getValue(), // Supported values: 'Year' (1-3), 'Month' (1-36), 'Week' (1-57), 'Day' (1-1095), 'Variable'
                value: recurringProductLineItem.product.custom.amzPayRecurringValue.toString() // Number of frequency units per billing cycle. For example, to specify a weekly cycle set unit to Week and value to 1. You must set value to 0 if you're using variable unit
            },
            amount: {
                amount: grossPrice && grossPrice.value ? grossPrice.value.toString() : '',
...
AmazonPayUtils.getPlatformId = function () {
    return 'A26EDUPKE5MR47';
};

AmazonPayUtils.generateAmazonSignInPayload = function () {
    var target = session.custom.amzSignInRedirect || '1';
    var payload;

    if (preferences.enableSFRAv6_1_0) {
        payload = {
            signInReturnUrl: URLUtils.https('AmazonPay-Login').toString(),
            storeId: preferences.amzPayStoreId,
            signInScopes: ['name', 'email', 'postalCode', 'shippingAddress', 'billingAddress', 'phoneNumber'],
            signInCancelUrl: target === '1' ? URLUtils.https('Login-Show').toString() : URLUtils.https('Checkout-Begin').toString()
        };
    } else {
        payload = {
            signInReturnUrl: URLUtils.https('AmazonPay-Login').toString(),
            storeId: preferences.amzPayStoreId,
            signInScopes: ['name', 'email', 'postalCode', 'shippingAddress', 'billingAddress', 'phoneNumber'],
            signInCancelUrl: target === '1' ? URLUtils.https('Login-Show').toString() : URLUtils.https('Checkout-Login').toString()
        };
    }
...
AmazonPayUtils.generateAmazonPayPayload = function () {
    ...
    if (currentBasket && currentBasket.productQuantityTotal != 0) {
        payload = {
            webCheckoutDetails: {
                checkoutReviewReturnUrl: preferences.amzPayCheckoutType.value === 'OneStepCheckout' ? URLUtils.https('AmazonPay-OneReview').toString() : URLUtils.https('AmazonPay-Review').toString(),
                checkoutResultReturnUrl: URLUtils.https('AmazonPay-Result').toString()
            },
            storeId: preferences.amzPayStoreId,
            paymentDetails: this.getPaymentDetail(currentBasket),
            chargePermissionType: this.isRecurringOrder(currentBasket) ? 'Recurring' : 'OneTime',
            scopes: ['name', 'email', 'phoneNumber', 'billingAddress']
        };

        // We set allowOvercharge to true, to enable the feature Charging Over Order Amount
        if (preferences.amzChargingOverOrderAmountEnabled && !this.isRecurringOrder(currentBasket)) {
            payload.paymentDetails.allowOvercharge = true;
        }

        if (payload.chargePermissionType === 'Recurring') {
            payload.recurringMetadata = this.getRecurringMetadata(currentBasket);
        }

        // If region is EU, then we add presentmentCurrency for multi-currency
        if (preferences.amzPayRegion.value === 'eu') {
            payload.paymentDetails.presentmentCurrency = this.getPaymentDetail(currentBasket).chargeAmount.currencyCode;
        }
    }
...
AmazonPayUtils.generateAmazonPayAdditionalButtonPayload = function () {
    var BasketMgr = require('dw/order/BasketMgr');
    var currentBasket = BasketMgr.getCurrentBasket();
    var shippingAddress;
    var addressDetails;
    ...
    if (currentBasket && currentBasket.productQuantityTotal != 0) {
        shippingAddress = currentBasket.shipments[0].shippingAddress;
        ...
        payload = {
            ...
            addressDetails: {
                ...
                county: '',
                district: '',
                stateOrRegion: shippingAddress.stateCode,
...
AmazonPayUtils.generateAmazonButtonSignature = function (payload) {
    var stringToSign = AMAZON_SIGNATURE_ALGORITHM + '\n' + hexAndHash(payload);
    var signature = calculateSignature(stringToSign);
...
AmazonPayUtils.getEstimatedOrderAmount = function () {
    var BasketMgr = require('dw/order/BasketMgr');

    var currentBasket = BasketMgr.getCurrentOrNewBasket();
    var grossPrice = currentBasket ? currentBasket.getTotalGrossPrice() : null;
    var estimatedOrderAmountObj = {};


    if (preferences.amzPayRegion.value === 'eu' || this.isRecurringOrder(currentBasket)) {
        return estimatedOrderAmountObj;
    }

    estimatedOrderAmountObj = {
        amount: grossPrice && grossPrice.value ? grossPrice.value.toString() : '',
        currencyCode: grossPrice && grossPrice.currencyCode ? grossPrice.currencyCode : session.currency.getCurrencyCode()
    }

    return JSON.stringify(estimatedOrderAmountObj);
}
